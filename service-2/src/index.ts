...
const startServer = async () => {
  try {
    ...
    server.listen(PORT, () => {
      console.log(`🚀 [server-2]: Server is running on ${PORT} 🔊`);
      process.env.SERVER_START_TIME = new Date().toISOString();
    });
  } catch (err) {
    console.error("💥[server-2]: Error starting server:", err);
    process.exit(1);
  }
};
...
const gracefulShutdown = () => {
  console.log(
    "🛑[server-2]: Received kill signal, starting graceful shutdown..."
  );

  server.close(async (err) => {
    if (err) {
      console.error("💥[server]: Error closing the server:", err);
      process.exit(1);
    }

    console.log("✅[server-2]: HTTP server closed.");
...
process.on("unhandledRejection", (reason, promise) => {
  console.error(
    "💥[server-2]: Unhandled Rejection at:",
    promise,
    "reason:",
    reason
  );
...
process.on("uncaughtException", (error) => {
  console.error("⚠️[server-2]: Uncaught Exception thrown:", error);
  gracefulShutdown();
});
